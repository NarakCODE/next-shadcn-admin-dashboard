...
function $convertDateTimeElement(domNode: HTMLElement): DOMConversionOutput | null {
  ...
  const parsedDate = parsed?.dat_df?.dfie_ts?.tv?.tv_s * 1000 || Date.parse(parsed?.dat_df?.dfie_dt || "");
...
export class DateTimeNode extends DecoratorTextNode {
  ...
  decorate(): JSX.Element {
    return <DateTimeComponent dateTime={this.getDateTime()} format={this.getFormat()} nodeKey={this.__key} />;
  }
}
