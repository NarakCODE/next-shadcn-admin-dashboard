...
function DataGridTableEmpty() {
  ...
  return (
    <tr>
      <td colSpan={Math.max(visibleColumnCount, 1)} className="py-6 text-center text-muted-foreground text-sm">
...
function DataGridTableRowSelect<TData>({ row }: { row: Row<TData> }) {
  return (
    <>
      <div
        className={cn("absolute inset-s-0 top-0 bottom-0 hidden w-[2px] bg-primary", row.getIsSelected() && "block")}
      />
      <Checkbox
